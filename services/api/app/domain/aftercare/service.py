...
_SAFETY_LETTER = (
    ...
    "或拨打 24 小时心理援助热线(北京 010-82951332,全国 400-161-9995)。"
    "在你联系到他们之前,我会一直在这里。"
)
...
def _fallback(persona: str) -> AftercareResponse:
    letter = _FALLBACK_LETTERS.get(persona, _FALLBACK_LETTERS["nini"])
...
async def generate_aftercare(request: AftercareRequest) -> AftercareResponse:
    ...
    system = _PERSONA_PROMPTS.get(persona, _PERSONA_PROMPTS["nini"])
    convo = "\n".join(
